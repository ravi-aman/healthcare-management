...
function Service() {
    const services = [
        ...
        { icon: <FaEllipsisH />, title: 'More...', description: 'Discover more services and features coming soon.' },
    ];

    return (
        <div className="relative top-[-120px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 p-4">
            {services.map((service, index) => (
                <div key={index} className="h-[200px] w-full bg-black rounded-lg shadow-lg p-5 hover:scale-105 transition-transform duration-300 ease-in-out hover:bg-gray-800">
